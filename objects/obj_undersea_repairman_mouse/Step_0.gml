...
if shield_hp > 0 && state != ENEMY_STATE.ACTING{
	sprite_index = spr_undersea_repairman_mouse_shield
	move_anim = 8
	attack_anim = 5
	move_speed = 0.6
}
else if shield_hp <= 0 && state != ENEMY_STATE.ACTING{
	sprite_index = spr_undersea_repairman_mouse
	move_anim = 8
	attack_anim = 4
	move_speed = 0.3
}
if shield_hp <= 0 && not placed && not armor_dropped{
	...
	inst.cgravity = 0.8
	inst.sprite_index = spr_undersea_repairman_ladder_1
	armor_dropped = true
}
event_inherited();
...
if not placed && state = ENEMY_STATE.ACTING{
	...
	if hp > maxhp * hurt_rate{
		if shield_hp > shield_max_hp * hurt_rate{
			...
			image_index = floor(timer/flash_speed) mod 6 + 6
		}
	}
	else{
		if shield_hp > shield_max_hp * hurt_rate{
			image_index = floor(timer/flash_speed) mod 6 + 12
		}
		else{
			image_index = floor(timer/flash_speed) mod 6 + 18
		}
	}
