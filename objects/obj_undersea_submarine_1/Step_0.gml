...
if hp > 0 && state != ENEMY_STATE.DEAD{
	if state == ENEMY_STATE.APPEAR{
		...
		if x <= get_world_position_from_grid(7,grid_row).x{
			...
			if grid_row == 0{
				sprite_index = spr_undersea_submarine_release_1
			}
	...
	if state == ENEMY_STATE.ACTING{
		...
		if anim_timer == 34*flash_speed-1{
			...
			instance_create_depth(summon_grid_pos.x+55,summon_grid_pos.y+38,depth,obj_undersea_panda_mouse)
			if grid_row > 0{
				instance_create_depth(summon_grid_pos.x+55,summon_grid_pos.y+38-global.grid_cell_size_y,depth,obj_undersea_panda_mouse)
			}
