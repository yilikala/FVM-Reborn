...
for(var i = 0 ; i < _mat_visible ; i++){
    ...
    // 高清素材尺寸减半：1.8 -> 0.9
    draw_sprite_ext(spr_craft_material, _mat_info.icon, x-752+i*84, y + 454, 0.9, 0.9, 0, c_white, 1)
    draw_set_halign(fa_right)
...
if button_select == 0{
	...
	for(var i = 0 ; i < ds_list_size(global.player_deck);i += 2){
		...
		if (card_slot_data != noone) {
			...
			if _unlocked_data.max_level > 0{
                // 高清星星插槽减半：1.4 -> 0.7
				draw_sprite_ext(spr_star_slot, _unlocked_data.max_level - 1, card_x-25, card_y-35, 0.7, 0.7, 0, c_white, 1);
			}
		}
	...
	if current_uprade_target_id != ""{
		...
		if card_data.max_level <= 15{
			...
			if input_spice_id != ""{
                ...
                // 高清材料减半：1.8 -> 0.9
				draw_sprite_ext(spr_craft_material,_spice_info.icon,x-455,y-20, 0.9, 0.9, 0, c_white, 1)
				draw_set_halign(fa_center)
				draw_set_valign(fa_middle)
				draw_set_font(font_number)
	...
	//绘制宝石强化UI背景（保留两槽位逻辑，gem_bg scale按上游降至 0.9）
	...
	if current_uprade_target_id != ""{
		...
		if get_gem_max_level(weapon_id) <= 14{
			...
			// 水晶槽：显示玩家投入（高清材料减半：1.8 -> 0.9）
			if input_crystal_id != ""{
				var _crystal_info = get_material_info(input_crystal_id)
				draw_sprite_ext(spr_craft_material, _crystal_info.icon, x-455, y-20, 0.9, 0.9, 0, c_white, 1)
				draw_set_halign(fa_center)
				draw_set_valign(fa_middle)
				draw_set_font(font_number)
				draw_set_colour(c_black)
	...
	if syn_input_spice_id != ""{
		...
		if !is_undefined(_rule){
			// 原料香料图标显示在上槽（高清材料减半：1.8 -> 0.9）
			var _spice_info = get_material_info(syn_input_spice_id)
			draw_sprite_ext(spr_craft_material, _spice_info.icon, x-305, y-40, 0.9, 0.9, 0, c_white, 1)
			draw_set_halign(fa_center)
			draw_set_valign(fa_middle)
			draw_set_font(font_number)
			draw_set_colour(c_black)
			...
			draw_set_color(c_white)
			// 产物图标显示在下槽（仅当未选四叶草时；若选了四叶草则优先显示四叶草）（高清材料减半：1.8 -> 0.9）
			if syn_input_clover_id == ""{
				var _out_info = get_material_info(_rule.output_id)
				draw_sprite_ext(spr_craft_material, _out_info.icon, x-305, y+110, 0.9, 0.9, 0, c_white, 1)
				draw_set_halign(fa_center)
				draw_set_valign(fa_middle)
				draw_set_font(font_yuan)
				draw_set_colour(c_black)
	...
	// 四叶草显示（高清材料减半：1.8 -> 0.9）
	if syn_input_clover_id != ""{
		var _clover_info = get_material_info(syn_input_clover_id)
		draw_sprite_ext(spr_craft_material, _clover_info.icon, x-305, y+110, 0.9, 0.9, 0, c_white, 1)
	}
	// 成功率
